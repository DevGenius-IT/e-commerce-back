...
class ListenRabbitMQRequestsCommand extends Command
{
    ...
    {
        $this->info('Starting RabbitMQ Request Listener for addresses-service...');

        try {
            // Initialize the request handler for this service
            $serviceUrl = env('APP_URL', 'http://localhost:8009');
            $this->requestHandler = new RabbitMQRequestHandlerService('addresses', $serviceUrl);
            ...
            $this->info('Listening for requests on queue: addresses.requests');
