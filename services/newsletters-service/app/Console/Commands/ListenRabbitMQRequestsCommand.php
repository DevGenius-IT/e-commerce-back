...
class ListenRabbitMQRequestsCommand extends Command
{
    ...
    {
        $this->info('Starting RabbitMQ Request Listener for newsletters-service...');

        try {
            // Initialize the request handler for this service
            $serviceUrl = env('APP_URL', 'http://localhost:8007');
            $this->requestHandler = new RabbitMQRequestHandlerService('newsletters', $serviceUrl);
            ...
            $this->info('Listening for requests on queue: newsletters.requests');
