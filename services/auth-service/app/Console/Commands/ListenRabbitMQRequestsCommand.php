...
class ListenRabbitMQRequestsCommand extends Command
{
    ...
    {
        $this->info('Starting RabbitMQ Request Listener for auth-service...');

        try {
            // Initialize the request handler for this service
            $serviceUrl = env('APP_URL', 'http://localhost:8001');
            $this->requestHandler = new RabbitMQRequestHandlerService('auth', $serviceUrl);
            ...
            $this->info('Listening for requests on queue: auth.requests');
