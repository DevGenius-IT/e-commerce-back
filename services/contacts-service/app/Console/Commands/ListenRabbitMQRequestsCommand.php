...
class ListenRabbitMQRequestsCommand extends Command
{
    ...
    {
        $this->info('Starting RabbitMQ Request Listener for contacts-service...');

        try {
            // Initialize the request handler for this service
            $serviceUrl = env('APP_URL', 'http://localhost:8010');
            $this->requestHandler = new RabbitMQRequestHandlerService('contacts', $serviceUrl);
            ...
            $this->info('Listening for requests on queue: contacts.requests');
