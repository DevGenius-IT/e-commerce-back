...
class ListenRabbitMQRequestsCommand extends Command
{
    ...
    {
        $this->info('Starting RabbitMQ Request Listener for questions-service...');

        try {
            // Initialize the request handler for this service
            $serviceUrl = env('APP_URL', 'http://localhost:8012');
            $this->requestHandler = new RabbitMQRequestHandlerService('questions', $serviceUrl);
            ...
            $this->info('Listening for requests on queue: questions.requests');
